# Hydra：基于 IETF RATS 的区块链机密计算环境通用可信远程证明协议框架
...
针对 **HyperEnclave、海光 CSV、Kunpeng、TDX、SGX2** 等主流 TEE 架构，开展底层安全机制与远程证明能力分析，具体包括：

- 构建主流 TEE 架构安全模型对比矩阵；
- 绘制不同 TEE 架构的信任链构建差异图谱；
- 评估各类 TEE 的远程证明方法与适用场景；
- 建立多维度 TEE 安全评估体系；
- 为区块链机密计算环境中的可信基础设施建设提供分析依据。
...
![图2 Hydra 远程证明协议交互流程图](./图2_交互流程图.png)
...
1. **设备注册与配置下发**  
   制造商通过智能合约更新 Shrub 根节点与索引值，向设备下发配置、路径和关联标签。

2. **设备状态提交**  
   设备周期性向验证合约提交证明证据，并在验证者发起挑战后返回相应的证明材料。

3. **验证者挑战与证据验证**  
   验证者向设备发送挑战，设备返回证据后，验证者对设备可信状态进行验证。

4. **链上状态查询与可信记录维护**  
   验证者可通过智能合约查询设备状态，合约返回设备可信状态结果，实现证明过程的透明化和可追溯。

5. **设备动态管理**  
   支持设备的动态添加与删除，并通过链上根节点更新机制维护设备可信集合的一致性。
