...
![图1 基于 IETF RATS 的区块链机密计算环境通用可信远程证明协议框架](./图1_架构图.png)
...
1. **混合 TEE 设备层**  
   支持 Intel SGX、ARM CCA、AMD SEV-SNP、海光 CSV 等异构 TEE 设备接入，并支持设备动态管理。

2. **证明生成与采集层**  
   负责采集设备度量值、身份凭据、证据材料与参考值，并生成可验证证明。

3. **RATS 通用可信远程证明协议层**  
   基于统一认证协议、交互式协同证明与非交互式协同证明，完成证据透明性、公共验证性、无信任验证与跨 TEE 互操作验证。

4. **零知识证明与验证层**  
   结合 Circom 电路构建、Snarkjs 证明生成、证明聚合与协同验证，实现安全分析与可信性证明。

5. **区块链与智能合约层**  
   通过链上验证、证明结果记录和可信服务调用，将远程证明结果与区块链可信执行逻辑结合。
...
- **图1** 展示了 Hydra 基于 IETF RATS 的分层架构，包括混合 TEE 设备层、证明采集层、通用远程证明协议层、零知识证明与验证层，以及区块链与智能合约层。
- **图2** 展示了制造商、设备、验证者与智能合约之间的交互流程，体现了设备配置、证明提交、状态查询和动态管理等关键环节。
